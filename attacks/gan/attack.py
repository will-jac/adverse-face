### from https://github.com/qizhangli/nobox-attacks/attack.py
...
def attack_ce_unsup(model, device, ori_img, attack_niters, eps, alpha, n_imgs, ce_method):
    model.eval()
    ori_img = ori_img.to(device)
    nChannels = 3
    tar_img = []
...
def attack_ce_proto(model, device, ori_img, attack_niters, eps, alpha, n_decoders, ce_method, n_imgs, prototype_inds):
    model.eval()
    ori_img = ori_img.to(device)
    tar_img = []
...
def main(
    ...
    for data_ind, (ori_img, _) in enumerate(dataloader):
        ...
        if mode =='prototypical':
            ...
            old_att_img = attack_ce_proto(model, device, ori_img, attack_niters = ce_niters,
            ...
            old_att_img = attack_ce_unsup(model, device, ori_img, attack_niters = ce_niters,
                                          eps = ce_epsilon, alpha=ce_alpha, n_imgs=n_imgs,
                                          ce_method=ce_method)

        att_img = attack_ila(model, device, ori_img, old_att_img, ila_niters, eps=epsilon)
